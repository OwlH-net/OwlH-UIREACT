...
import { EditNode, ToggleAddNodeForm, SaveSelectedTags } from '../../../store/node/actions'
...
const AddNodeForm = (props) => {
  ...
  //add node button
  const getData = () => {
    //check tags selected. If is empty, get node tags by default
    if (props.tagsSelected.length == 0) {
      props.saveSelectedTags(props.nodeToEdit.tags)
    }
    ...
    if(props.nodeToEdit.id == undefined || props.nodeToEdit.id == null || props.nodeToEdit.id == ""){
      props.enroll(nodeData)
    }else{
      nodeData.Node.uuid = props.nodeToEdit.uuid
      props.editNode(nodeData)
    }
...
const mapDispatchToProps = (dispatch) => ({
  ...
  addAlertToAlertList: (error) => dispatch(AddAlertToAlertList(error)),
  saveSelectedTags: (tags) => dispatch(SaveSelectedTags(tags)),
})
